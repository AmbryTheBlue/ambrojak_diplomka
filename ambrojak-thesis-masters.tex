...
% \workinfo   {\url{https://github.com/AmbryTheBlue/ambrojak_diplomka}} % Does not look good and it is not important
...
Three distinct detection approaches are proposed and evaluated: an approach based on classification, out-of-distribution detection, and confidence-based anomaly scoring. 

The system is implemented as a modular microservice application using CodeNOW and Docker. Although parts of the real-time detection pipeline are functional and integrated with the Metrans infrastructure, the full deployment remains incomplete, primarily due to unresolved differences between the MIS2 and MIS3 data formats. Nonetheless, the work provides a solid experimental foundation and a clear path toward operational integration.
...
Jsou navrženy a vyhodnoceny tři odlišné přístupy k detekci anomálií — klasifikační modely, detekce anomálií na základě četnosti a detekce na základě míry jistoty modelu. 

Systém je implementován jako modulární aplikace založená na mikroservisní architektuře s využitím nástrojů CodeNOW a Docker. Ačkoli některé části pipeline pro detekci v reálném čase již fungují a byly napojeny na infrastrukturu Metrans, plné nasazení zatím nebylo dokončeno - především kvůli nevyřešeným rozdílům mezi datovými strukturami MIS2 a MIS3. I tak práce poskytuje pevný experimentální základ a jasný směr pro budoucí integraci do provozu.
...
\keywordsEN {%
Anomaly detection in tabular data, Misclassification Detection, Frequency-Based Outlier Detection, Confidence-Based Anomaly Detection, Intermodal transport
\rfc{Zkontrolovat keywords, doplnit kdyžtak něco}
}
\keywordsCZ {%
Detekce anomálií v tabulkových datech, Detekce špatné klasifikace, Detekce anomálií na základě četností, Detekce anomálií na základě míry jistoty, Intermodální doprava
