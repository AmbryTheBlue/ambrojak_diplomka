...
\label[anomalies]
\chap Data Analysis and Specification of the Problem Area
In this chapter, we first define what is intermodal transport. Seond we look more specifically at Metrans and where do we get our data from. And finally we specify ``common'' anomalies (as far as rare anomalies can be common) in Metrans transportation orders that cause significant financial losses. 
...
Only a small subset of transport orders include heating (see Table \ref[distributionHasHeating]), and their distribution across these types is highly imbalanced; see Table \ref [distributionHeatingType]. This unevenness may indicate operational anomalies or data entry errors.
...
\caption/f Distribution of Temperatures, note this are not °C but label created by LabelEncoder to give us an insight into number of different classes
\medskip
...
\picw=15cm \cinspic imgs/Distribution-Additional-reference-export.pdf
\caption/f Distribution of presence of Chassis Type (export, for import see \ref[distributionChassisIm])
\medskip
